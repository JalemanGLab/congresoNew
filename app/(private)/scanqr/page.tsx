  ...
  return (
    <div className="text-background w-full h-full flex flex-col items-center justify-center px-7 gap-7">
      ...
      <div className="text-sm sm:text-base md:text-lg text-gray-400 text-center">
        Coloca el código QR dentro del marco para escanearlo. Evita movimientos
        bruscos para obtener resultados rápidamente.
      </div>

      {/* Mostrar la imagen QR solo cuando no se está escaneando */}
      {!isScanning && (
        <img src={qrImage.src} alt="qr" width="200" height="200" />
      )}
      ...
      <button
        ...
        } bg-[#073118] hover:bg-[#073118] w-full max-w-[400px] sm:w-[400px] text-white font-semibold  transition-all duration-300 px-4 py-2 rounded-md flex items-center justify-center`}
        disabled={isLoading}
      >
        {isLoading ? (
          <>
      ...
      <div className="hidden md:block text-sm text-gray-400 text-center mt-4">
        Nota: El escaneo solo funciona desde un teléfono móvil. Si estás en un
        PC, usa tu teléfono para acceder a esta página y escanear el código QR.
      </div>
    </div>
  );
}
